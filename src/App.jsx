...
function MainApp({ currentUser, onLogout, showToast }) {
  ...
  // Tutup semua dropdown kalau tap di luar
  useEffect(() => {
    const handler = () => {
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#eef2f7",
      ...
      <header style={{background:"#0f2e50",padding:"0 16px",paddingTop:"env(safe-area-inset-top)",
        ...
        {/* Notif */}
        <div style={{position:"relative"}}>
          <button onClick={e=>{e.stopPropagation();setShowNotif(v=>!v);setShowUserMenu(false);}}
            style={{background:"rgba(255,255,255,0.1)",border:"none",color:"#fff",
              width:36,height:36,borderRadius:10,cursor:"pointer",fontSize:18,
          ...
          {showNotif && (
            <div onClick={e=>e.stopPropagation()} style={{position:"absolute",right:0,top:44,width:260,background:"#fff",
        ...
        {/* Avatar + User Menu */}
        <div style={{position:"relative"}}>
          <button onClick={e=>{e.stopPropagation();setShowUserMenu(v=>!v);setShowNotif(false);}}
          ...
          {showUserMenu && (
            <div onClick={e=>e.stopPropagation()} style={{position:"absolute",right:0,top:42,width:220,background:"#fff",
              borderRadius:14,boxShadow:"0 8px 32px rgba(0,0,0,0.18)",zIndex:200,overflow:"hidden"}}>
