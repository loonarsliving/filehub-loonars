...
// ── UPLOAD SHEET dengan Google Drive ──
...
,
    ...
    const timer=setInterval(()=>{
      try{
        ...
        if(url.includes("access_token")){
          ...
          if(token){setAccessToken(token);setAuthDone(true);clearInterval(timer);w.close();}
        }
      }catch(e){}
  ...
  const addFiles=list=>{
    setFiles(Array.from(list).map(f=>({file:f,name:f.name,size:(f.size/1024).toFixed(0)+" KB",status:"ready"})));
  };

  const doUpload=async()=>{
    if(!accessToken){toast("Login Google dulu!","error");return;}
    if(files.length===0){toast("Pilih file dulu!","error");return;}
    setUploading(true);
    const updated=[...files];
    for(let i=0;i<updated.length;i++){
      ...
      setFiles([...updated]);
      try{
        const result=await uploadToDrive(updated[i].file,folderId,accessToken);
        if(result.id){
          updated[i].status="done";
          updated[i].viewUrl=result.webViewLink;
          await sb.insert("activity_log",{branch_id:branch,action:"upload",file_name:updated[i].name,metadata:{driveId:result.id,size:updated[i].size}});
        }else{
    ...
    if(doneCount>0)toast(`${doneCount} file berhasil diupload ke Drive!`);
  };
    ...
    <Sheet onClose={onClose}>
      <div style={{padding:"14px 20px 24px"}}>
        ...
        <div style={{fontSize:12,color:"#7a8a9a",marginTop:3,marginBottom:16}}>
          📁 {s?s.name:b.name} &nbsp;·&nbsp;
          {authDone
            ? <span style={{color:"#2ecc71",fontWeight:700}}>● Google Terhubung</span>
            : <span style={{color:"#e67e22",fontWeight:700}}>● Perlu Login Google</span>}
        </div>

        {!authDone&&(
          <button onClick={loginGoogle} style={{width:"100%",padding:"13px",borderRadius:12,border:"none",background:"#fff",cursor:"pointer",fontWeight:700,fontSize:14,color:"#0f2e50",border:"1.5px solid #dde3ec",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
            <span style={{fontSize:20}}>🔑</span> Login dengan Google untuk Upload
          </button>
        )}
            ...
            {files.map((f,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #f0f2f5"}}>
                ...
                <span style={{fontSize:16}}>
                  {f.status==="ready"?"⏸️":f.status==="uploading"?"⏳":f.status==="done"?"✅":"❌"}
                </span>
              </div>
            ))}
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <button onClick={doUpload} disabled={uploading||!authDone||files.length===0}
            style={{padding:13,borderRadius:12,border:"none",
              background:uploading||!authDone||files.length===0?"#b0c0d0":"#0f2e50",
              color:"#fff",cursor:uploading?"not-allowed":"pointer",fontWeight:700,fontSize:14}}>
            {uploading?"⏳ Mengupload...":"⬆ Upload ke Drive"}
          </button>
        </div>
      </div>
    </Sheet>
