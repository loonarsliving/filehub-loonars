...
const drive = {
  async listFiles(folderId, token) {
    ...
    const res = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,mimeType,size,webViewLink,modifiedTime,owners)&pageSize=100`,
...
const tokenStore = {
  save(token, expiry) { localStorage.setItem("g_token", token); localStorage.setItem("g_expiry", expiry); },
  get() {
    const token = localStorage.getItem("g_token");
    const expiry = parseInt(localStorage.getItem("g_expiry") || "0");
    if (!token || Date.now() > expiry) { localStorage.removeItem("g_token"); localStorage.removeItem("g_expiry"); return null; }
    return token;
  },
  clear() { localStorage.removeItem("g_token"); localStorage.removeItem("g_expiry"); }
};
...
const MIME={
  ...
  "video/mp4":{icon:"🎬",color:"#c0392b",bg:"#fdecea",label:"MP4"},
};
...
// ── UPLOAD SHEET ──
function UploadSheet({onClose,branch,sub,toast,gToken,onTokenUpdate,onRefresh}){
  ...
  const doUpload=async()=>{
    ...
    for(let i=0;i<updated.length;i++){
      ...
      try{
        const res=await drive.uploadFile(updated[i].file,folderId,gToken);
        if(res.id){
          updated[i].status="done";
          await sb.insert("activity_log",{branch_id:branch,action:"upload",file_name:updated[i].name,metadata:{driveId:res.id,folderId}});
    ...
    if(done>0){
      toast(`${done} file berhasil diupload ke Drive!`);
...
function MainApp({me,onLogout,toast}){
  ...
  const [gToken,setGToken]=useState(()=>tokenStore.get());

  // Access requests
  const [pendingReqs,setPendingReqs]=useState([]);
  const [activeReq,setActiveReq]=useState(null);

  // Load files from Drive for all branches
  const loadAllFiles=useCallback(async(token)=>{
    ...
    try{
      ...
      for(const b of BRANCHES.filter(x=>x.id!=="all")){
        const folderId=DRIVE_FOLDERS[b.id];
        if(folderId&&folderId!=="root"){
          const files=await drive.listFiles(folderId,token);
          results[b.id]=files.map(f=>({...f,branch:b.id,sub:null}));
        } else {
          results[b.id]=[];
        }
      }
  ...
  const loadRequests=async()=>{
    try{
      const data=await sb.query("access_requests",{select:"*",filter:"status=eq.pending",order:"created_at.desc",limit:20});
      if(Array.isArray(data))setPendingReqs(data);
    }catch(e){}
  };

  useEffect(()=>{
    if(gToken)loadAllFiles(gToken);
    loadRequests();
  ...
  const approveReq=async()=>{
    ...
    toast(`Akses disetujui — ${activeReq.file_name}`);
  ...
  const rejectReq=async()=>{
    ...
    toast(`Akses ditolak — ${activeReq.file_name}`,"error");
    setPendingReqs(p=>p.filter(r=>r.id!==activeReq.id));
    setActiveReq(null);
  };
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#eef2f7",minHeight:"100dvh",display:"flex",flexDirection:"column",maxWidth:600,margin:"0 auto"}}>
      ...
      {/* Drive status bar */}
      {!gToken&&(
        <div style={{background:"#fff8e6",borderBottom:"1px solid #fde8a0",padding:"8px 16px",display:"flex",alignItems:"center",gap:10,justifyContent:"space-between"}}>
          <span style={{fontSize:12,color:"#7d4e00",fontWeight:600}}>⚠️ Login Google untuk lihat & upload file dari Drive</span>
          <button onClick={()=>setSheetUpload(true)} style={{padding:"5px 12px",borderRadius:8,border:"none",background:"#f39c12",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Login</button>
        </div>
      )}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px 8px",flexShrink:0}}>
        <div>
          ...
          <div style={{fontSize:12,color:"#7a8a9a",marginTop:1}}>{loadingFiles?"Memuat dari Drive...":filtered.length+" file"}</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>gToken&&loadAllFiles(gToken)} style={{background:"#eef2f7",border:"none",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:14,color:"#0f2e50"}}>↻</button>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 12px 90px"}}>
        {loadingFiles?(
          <div style={{padding:48,textAlign:"center",color:"#9aacbe"}}><div style={{fontSize:36,marginBottom:10}}>⏳</div><div style={{fontWeight:600}}>Memuat file dari Google Drive...</div></div>
        ):!gToken?(
          <div style={{padding:48,textAlign:"center",color:"#9aacbe"}}><div style={{fontSize:36,marginBottom:10}}>🔐</div><div style={{fontWeight:600}}>Login Google untuk lihat file Drive</div><button onClick={()=>setSheetUpload(true)} style={{marginTop:16,padding:"10px 24px",borderRadius:12,border:"none",background:"#0f2e50",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>Login Google</button></div>
        ):view==="list"?(
          <div style={{background:"#fff",borderRadius:14,border:"1px solid #e0e8f0",overflow:"hidden"}}>
            ...
            {filtered.length===0&&<div style={{padding:48,textAlign:"center",color:"#9aacbe"}}><div style={{fontSize:36,marginBottom:10}}>📂</div><div style={{fontWeight:600}}>Tidak ada file di cabang ini</div></div>}
          </div>
        ):(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      {/* SHEETS */}
      {sheetSearch&&<Sheet onClose={()=>setSheetSearch(false)}><div style={{padding:"14px 20px 24px"}}><div style={{fontWeight:700,fontSize:16,color:"#0f2e50",marginBottom:14}}>🔍 Cari File</div><input value={search} onChange={e=>setSearch(e.target.value)} autoFocus placeholder="Nama file..." style={{width:"100%",padding:"13px 14px",borderRadius:12,border:"1.5px solid #dde3ec",fontSize:14,outline:"none",boxSizing:"border-box"}}/>{search&&<div style={{marginTop:12,fontSize:13,color:"#7a8a9a"}}>{filtered.length} hasil ditemukan</div>}<button onClick={()=>{setSearch("");setSheetSearch(false);}} style={{marginTop:14,width:"100%",padding:13,borderRadius:12,border:"none",background:"#eef2f7",cursor:"pointer",fontWeight:700,color:"#5a6a7a",fontSize:14}}>Tutup</button></div></Sheet>}

      {sheetNotif&&(
        <Sheet onClose={()=>setSheetNotif(false)}>
          <div style={{padding:"14px 20px 24px"}}>
            <div style={{fontWeight:700,fontSize:16,color:"#0f2e50",marginBottom:16}}>🔔 Notifikasi {pendingReqs.length>0&&<span style={{background:"#e74c3c",color:"#fff",borderRadius:10,padding:"2px 8px",fontSize:12,marginLeft:8}}>{pendingReqs.length}</span>}</div>
            {pendingReqs.length===0?<div style={{textAlign:"center",padding:"24px 0",color:"#9aacbe",fontSize:14}}>Tidak ada notifikasi baru</div>:
            pendingReqs.map(r=>(
              <div key={r.id} onClick={()=>{setActiveReq(r);setSheetNotif(false);}} style={{background:"#fff8e6",borderRadius:12,padding:"12px 14px",marginBottom:10,cursor:"pointer",border:"1px solid #fde8a0"}}>
                <div style={{fontWeight:700,color:"#7d4e00",fontSize:13}}>🔐 Permintaan Akses File</div>
                <div style={{fontSize:12,color:"#a0700a",marginTop:4}}>{r.file_name}</div>
                <div style={{fontSize:11,color:"#c0a060",marginTop:4}}>Dari: {r.requester_branch} · {fd(r.created_at)}</div>
              </div>
            ))}
            <button onClick={()=>setSheetNotif(false)} style={{width:"100%",padding:13,borderRadius:12,border:"none",background:"#eef2f7",cursor:"pointer",fontWeight:700,color:"#5a6a7a",fontSize:14,marginTop:8}}>Tutup</button>
          </div>
        </Sheet>
      )}

      {sheetUser&&<Sheet onClose={()=>setSheetUser(false)}><div style={{padding:"14px 20px 24px"}}><div style={{display:"flex",alignItems:"center",gap:14,paddingBottom:16,borderBottom:"1px solid #eef0f4",marginBottom:14}}><div style={{width:50,height:50,borderRadius:"50%",background:me.role==="master"?"#f39c12":"#1e4878",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:20}}>{me.name[0].toUpperCase()}</div><div><div style={{fontWeight:700,fontSize:16,color:"#1a2b45"}}>{me.name}</div><div style={{fontSize:13,color:"#7a8a9a",marginTop:2}}>{me.email}</div><span style={{background:me.role==="master"?"#fff8e6":"#eaf3fb",color:me.role==="master"?"#7d4e00":"#1a5276",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:10,display:"inline-block",marginTop:6}}>{me.role==="master"?"👑 Master":"🔑 Admin"}</span></div></div>{gToken&&<div style={{background:"#eafaf1",borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#1e8449",fontWeight:600}}>✅ Google Drive Terhubung</div>}{gToken&&<button onClick={()=>{tokenStore.clear();setGToken(null);setDriveFiles({});toast("Google logout","error");setSheetUser(false);}} style={{width:"100%",padding:"12px 16px",borderRadius:12,border:"none",background:"#fff8e6",cursor:"pointer",textAlign:"left",fontSize:14,color:"#e67e22",fontWeight:600,marginBottom:10}}>🔓 Disconnect Google Drive</button>}{master&&<button onClick={()=>{setSheetUser(false);setPageUsers(true);}} style={{width:"100%",padding:"14px 16px",borderRadius:12,border:"none",background:"#f0f4fa",cursor:"pointer",textAlign:"left",fontSize:15,color:"#0f2e50",fontWeight:600,marginBottom:10,display:"flex",alignItems:"center",gap:10}}>👥 Kelola User</button>}<button onClick={()=>{setSheetUser(false);onLogout();}} style={{width:"100%",padding:"14px 16px",borderRadius:12,border:"none",background:"#fdecea",cursor:"pointer",textAlign:"left",fontSize:15,color:"#c0392b",fontWeight:600,display:"flex",alignItems:"center",gap:10}}>🚪 Keluar</button></div></Sheet>}
        ...
        <Sheet onClose={()=>setPreview(null)}>
          <div style={{padding:"14px 20px 24px"}}>
            ...
            <div style={{background:"#f7f9fc",borderRadius:12,padding:"12px 14px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {[{l:"PEMILIK",v:preview.owners?.[0]?.displayName||"Loonars"},{l:"DIUBAH",v:fd(preview.modifiedTime)},{l:"CABANG",v:<Badge branchId={preview.branch} small/>},{l:"UKURAN",v:fs(preview.size)}].map((r,i)=><div key={i}><div style={{fontSize:10,color:"#8a9ab0",fontWeight:700,letterSpacing:0.8}}>{r.l}</div><div style={{fontSize:13,color:"#1a2b45",fontWeight:600,marginTop:3}}>{r.v}</div></div>)}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <a href={preview.webViewLink} target="_blank" rel="noreferrer" style={{padding:13,borderRadius:12,border:"1.5px solid #dde3ec",background:"#fff",fontWeight:700,color:"#0f2e50",fontSize:14,textDecoration:"none",textAlign:"center",display:"block"}}>⬇ Download</a>
              <a href={preview.webViewLink} target="_blank" rel="noreferrer" style={{padding:13,borderRadius:12,border:"none",background:"#0f2e50",color:"#fff",fontWeight:700,fontSize:14,textDecoration:"none",textAlign:"center",display:"block"}}>🔗 Buka Drive</a>
            </div>
          </div>
        </Sheet>
      )}
      ...
      {activeReq&&(
        <Sheet onClose={()=>setActiveReq(null)}>
          <div style={{padding:"14px 20px 24px"}}>
            <div style={{background:"#fff8e6",borderRadius:12,padding:"14px",marginBottom:14,border:"1px solid #fde8a0",display:"flex",gap:12,alignItems:"center"}}><span style={{fontSize:26}}>🔐</span><div><div style={{fontWeight:700,fontSize:15,color:"#7d4e00"}}>Permintaan Akses File</div><div style={{fontSize:12,color:"#a0700a",marginTop:2}}>Perlu persetujuan Master</div></div></div>
            <div style={{background:"#f7f9fc",borderRadius:12,padding:14,marginBottom:12}}>
              <div style={{fontSize:11,color:"#8a9ab0",fontWeight:700,letterSpacing:0.8,marginBottom:8}}>FILE DIMINTA</div>
              <div style={{fontWeight:700,color:"#1a2b45",fontSize:14}}>{activeReq.file_name}</div>
              <div style={{fontSize:12,color:"#7a8a9a",marginTop:4}}>Dari cabang: <strong>{activeReq.requester_branch}</strong></div>
            </div>
            <div style={{background:"#fef3f3",borderRadius:10,padding:"10px 14px",border:"1px solid #fad4d4",fontSize:12,color:"#7a2a2a",marginBottom:14}}>⚠️ Pastikan Anda yakin sebelum menyetujui akses.</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <button onClick={rejectReq} style={{padding:13,borderRadius:12,border:"1.5px solid #fad4d4",background:"#fff",cursor:"pointer",fontWeight:700,color:"#c0392b",fontSize:14}}>✕ Tolak</button>
              <button onClick={approveReq} style={{padding:13,borderRadius:12,border:"none",background:"#1e8449",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:14}}>✓ Setujui</button>
            </div>
          </div>
        </Sheet>
      )}
